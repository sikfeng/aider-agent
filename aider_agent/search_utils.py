...
def find_javascript_files(dir_path: str) -> list[str]:
    ...
    Skips files that are obviously not from the source code, such as third-party library code.

    Args:
        dir_path (str): Path to the directory.
    Returns:
        List[str]: List of .js file paths. These paths are ABSOLUTE paths!
    """
    ...
    for file in js_files:
        ...
        if rel_path.startswith("build"):
            continue
        if rel_path.startswith("node_modules"):
            continue
...
def find_typescript_files(dir_path: str) -> list[str]:
    """Get all .ts and .tsx files recursively from a directory.
    ...
    Args:
        dir_path (str): Path to the directory.
    Returns:
        List[str]: List of .ts and .tsx file paths. These paths are ABSOLUTE paths!
    """

    ts_files = glob.glob(pjoin(dir_path, "**/*.ts"), recursive=True)
    tsx_files = glob.glob(pjoin(dir_path, "**/*.tsx"), recursive=True)
...
def parse_file(file_full_path: str) -> tuple[list, dict, list] | None:
    """
    ...
    try:
        ...
        if lang == "python":
            # Use AutoCodeRover's original python parser
            #return parse_python_file(file_full_path)
            pass
    ...
    def traverse_node(node, parent_class=None):
        ...
        elif node.grammar_name in ["method_definition", "function_declaration"]:
            # TODO: doesn't index js functions of grammar "member_expression", "variable_declarator" or "pair"
            func_name = node.child_by_field_name('name').text.decode('utf8')
            start_line = node.start_point[0] + 1
            end_line = node.end_point[0] + 1
            
            if parent_class:
                class_to_funcs[parent_class].append((func_name, start_line, end_line))
            else:
                top_level_funcs.append((func_name, start_line, end_line))
...
'''def parse_file(file_full_path: str) -> tuple[list, dict, list] | None:
    ...
    try:
        ...
        parser = get_parser(lang)
        query_scm = resources.files("aider").joinpath("queries", f"tree-sitter-{lang}-tags.scm")
        ...
        for node, tag in captures:
            if tag == "name.definition.class":
                ...
                if node.parent.type in ["class_declaration", "class_definition"]:
                    start_line = node.parent.start_point[0] + 1
                    end_line = node.parent.end_point[0] + 1
                else:
                    print("WARN: parent of class has unknown type", node.parent.type)
                    start_line = node.start_point[0] + 1
                    end_line = node.end_point[0] + 1
                classes.append((class_name, start_line, end_line))

                class_methods = []
                captures2 = query.captures(node)
                for node2, tag2 in captures2:
                    if tag2 in ["name.definition.method", "name.definition.function"]:
                        method_name = node2.text.decode('utf8')
                        if node2.parent.type in ["method_definition", "function_definition"]:
                            start_line2 = node2.parent.start_point[0] + 1
                            end_line2 = node2.parent.end_point[0] + 1
                        else:
                            print("WARN: parent of method has unknown type", node.parent.type)
                            start_line2 = node2.start_point[0] + 1
                            end_line2 = node2.end_point[0] + 1
                        class_methods.append((method_name, start_line2, end_line2))
                class_to_methods[class_name] = class_methods
            elif tag == "name.definition.function":
                # skip if the function is inside a class (should already be added as a class method)
                parent_ = node.parent
                while not parent_.type == "program":
                    if parent_.type in ["class_declaration", "class_definition"]:
                        break
                    parent_ = parent_.parent
                if not parent_.type == "program":
                    continue
                
                class_name = node.text.decode('utf8')
                if node.parent.type in ["function_declaration", "function", "function_definition"]:
                    start_line = node.parent.start_point[0] + 1
                    end_line = node.parent.end_point[0] + 1
                elif node.parent.type in ["member_expression", "variable_declarator", "pair"]:
                    # TODO: line numbers for parent type `pair` is too wide, but idk how to be more specific
                    start_line = node.parent.parent.start_point[0] + 1
                    end_line = node.parent.parent.end_point[0] + 1
                else:
                    print("WARN: parent of function has unknown type", node.parent.type)
                    start_line = node.start_point[0] + 1
                    end_line = node.end_point[0] + 1
                top_level_funcs.append((class_name, start_line, end_line))
            else:
                continue
        
        print(class_to_methods)
        # Return the collected information
        return classes, class_to_methods, top_level_funcs
...
def get_func_snippet_in_python_class(
...
def get_func_snippet_in_class(
    ...
    if lang == "python":
        # Use AutoCodeRover's original python parser
        return get_func_snippet_in_python_class(file_full_path, class_name, func_name, include_lineno)
        pass
    ...
    def traverse_node(node, found_class = False):
        if node.grammar_name in ["class_declaration", "class_definition"] and node.child_by_field_name('name').text.decode('utf8') == class_name:
            found_class = True
        elif found_class and node.grammar_name in ["method_definition", "function_declaration"]:
            # TODO: doesn't index js functions of grammar "member_expression", "variable_declarator" or "pair"
        ...
        for child in node.children:
            traverse_node(child, found_class)
...
def get_func_snippet_with_code_in_python_file(file_full_path: str, code_str: str) -> list[str]:
...
def get_func_snippet_with_code_in_file(file_full_path: str, code_str: str) -> list[str]:
    ...
    lang = filename_to_lang(file_full_path)
    if lang == "python":
        # Use AutoCodeRover's original function
        return get_func_snippet_with_code_in_python_file(file_full_path)
        pass
    ...
    def traverse_node(node):
        if node.grammar_name in ["method_definition", "function_declaration"]:
            # TODO: doesn't index js functions of grammar "member_expression", "variable_declarator" or "pair"
            func_start_lineno = node.start_point[0] + 1
            func_end_lineno = node.end_point[0] + 1
            assert func_end_lineno is not None
            func_code = get_code_snippets(
                file_full_path, func_start_lineno, func_end_lineno
            )
...
# TODO: I'm not sure how to handle function signatures for arbitrary languages
# also this is very highly dependent on whether there even was function signatures
# for the purpose of our code snipping agent, I think we don't really need these
...
def extract_python_class_sig_from_ast(class_ast: ast.ClassDef) -> list[int]:
...
def extract_class_sig_from_ast(class_node) -> list[int]:
    """Extract the class signature from the AST.
    ...
    """
    # STEP (1): extract the class signature
    sig_start_line = class_node.start_point[0] + 1
    if class_node.child_by_field_name("body"):
        # has body
        body_start_line = class_node.child_by_field_name("body").start_point[0] + 1
        sig_end_line = class_node.start_point[0] + 1
    else:
        # no body
        sig_end_line = class_node.end_point[0] + 1
    assert sig_end_line is not None
    sig_lines = list(range(sig_start_line, sig_end_line + 1))

    # STEP (2): extract the function signatures and assign signatures
    for stmt in class_node.child_by_field_name("body"):
        if stmt.grammar_name in ["function_definition", "method_definition"]:
            sig_lines.extend(extract_func_sig_from_ast(stmt))
        elif isinstance(stmt, ast.Assign):
            # for Assign, skip some useless cases where the assignment is to create docs
            stmt_str_format = ast.dump(stmt)
            if "__doc__" in stmt_str_format:
                continue
            # otherwise, Assign is easy to handle
            assert stmt.end_point[0] is not None
            assign_range = list(range(stmt.start_point[0] + 1, stmt.end_point[0] + 1 + 1))
            sig_lines.extend(assign_range)

    return sig_lines


def get_python_class_signature(file_full_path: str, class_name: str) -> str:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef) and node.name == class_name:
            ...
            relevant_lines = extract_python_class_sig_from_ast(node)
            break
...
def get_class_signature(file_full_path: str, class_name: str) -> str:
    """Get the class signature using tree-sitter.
    ...
    if lang == "python":
        # Use AutoCodeRover's original function
        return get_python_class_signature(file_full_path, class_name)
        pass
    ...
    for node in tree:
        if node.grammar_name in ["class_definition", "class_declaration"] and node.child_by_field_name('name') == class_name:
