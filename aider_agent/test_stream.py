...
# Initialize logging
LOG_CONFIG['handlers']['fileHandler']['filename'] = utils.get_absolute_path("/tmp/test_stream.log")
...
async def test_websocket_endpoint(uri, method, params=None):
    async with websockets.connect(uri, ping_interval=None) as websocket:
        ...
        while True:
            ...
            if partial_response_data["result"] == "</eos_token>":
...
async def test():
    ...
    for repo_dir in external_repos:
        logger.info(f"Initializing external repo agent for {repo_dir}")
    ...
    logger.info(f"Generating subtasks for task: {task}")
    ...
    for subtask in subtasks:
        logger.info(f"Running subtask: {subtask}")
